...
import matplotlib  # added by Ziyuan
matplotlib.use('agg') # added by Ziyuan, backend mode, otherwise gives error when importing pyplot/pylab
import matplotlib.pyplot as plt # added by Ziyuan according to https://stackoverflow.com/questions/36327134/matplotlib-error-no-module-named-tkinter
import pylab as pl
...
def printpage():
    print "Script for calculating dvh metrics"
    print "python ./dvhcalc.py refplan surID"
    quit()
...
def calculate_dvh(structure,dose,limit=None, calculate_full_volume=True, use_structure_extents=False,\
 interpolation_resolution=None, interpolation_segments_between_planes=0,callback=None):
...
def calDVHmetrics(rtssfile,rtdosefile,outputfile=None):
    ...
    for key, structure in RTstructures.iteritems():
        ...
        if OARname in OARs:
            ...
            mean_dose = np.sum(diff_dvh.counts*diff_dvh.bins[1:])/diff_dvh.volume    #integra of the differential dvh, and divide the volume
            ...
            if calcdvhs[key].volume> 0:
                v2cc = 2/calcdvhs[key].volume * 100 # relative volume of 2cc to the OAR
                binsabove2cc = calcdvhs[key].bins[np.where(calcdvhs[key].counts > v2cc)]
                D2cc = calcdvhs[key].bins[np.where(calcdvhs[key].counts > v2cc)][-1]
            else:
                D2cc = 0
            strDVH = strDVH + patient + ', ' + structure['name'] + ', ' + str(np.round(calcdvhs[key].volume,2)) + ', ' + str(np.round(mean_dose,2)) + ', '+ str(np.round(V5,2)) + ', ' + str(np.round(V10,2)) + ', '+ str(np.round(D2cc,2)) + '\n'
    if outputfile is None:
        ...
        if os.stat(outputfile).st_size == 0:
            fl.write("patient, OAR, volumn(cc), Mean dose(Gy), V5(%), V10(%), D2cc(Gy)\n")     
        fl.write(strDVH)
        fl.close()
        return None 

def passfractionOAR(rtssfile,gamma,outputfile=None):
    RTss = dicomparser.DicomParser(rtssfile)
    #RTdose = dicomparser.DicomParser("testdata/rtdose.dcm") 
    RTstructures = RTss.GetStructures()
    patient = rtssfile[rtssfile.rfind('RS.')+3: rtssfile.rfind('.dcm')]
    ...
    for key, structure in RTstructures.iteritems():
        OARname = structure['name'].replace("_", " ")
        if OARname in OARs:
            passfraction[key] = dvhcalc.get_pass(rtssfile, gamma, key)
            #print(structure['name'])
            passf, gamma_mean = np.sum(diff_dvh.counts*diff_dvh.bins[1:])/diff_dvh.volume    #integra of the differential dvh, and divide the volume
            strpf = strpf + patient + ', ' + structure['name'] + ', ' + passf + gamma_mean +'\n'
    if outputfile is None:
        print "patient, OAR, passfraction, mean gamma\n", strDVH
        return strDVH
    else:
        fl = open(outputfile,"a")
        if os.stat(outputfile).st_size == 0:
            fl.write("patient, OAR, passfraction, mean gamma\n")     
        fl.write(strpf)
        fl.close()
        return None 

def get_gamma(structure,
            gamma,
            roi,
            limit=None,
            calculate_full_volume=True,
...
def main():
    ...
    #rtssfile = '../data_RS/RS.XXX.dcm'
    #rtdosefile = '../auto_Dose/RD.00.refXXX.XXX.dcm'
    ...
    refplan= sys.argv[1]
    surID= sys.argv[2]
    rtssfile = '../data/data_RS/RS.'+surID+'.dcm'
    rtdosefile = '../data/auto_Dose/RD.00.ref'+refplan+'.'+surID+'.dcm'
    try:
        RTss = dicomparser.DicomParser(rtssfile)
        RTstructures = RTss.GetStructures()
        calcdvhs = {}
        print "example on calculating DVH metrcis for ", rtdosefile
        strDVH = calDVHmetrics(rtssfile,rtdosefile)
